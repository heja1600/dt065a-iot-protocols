...
public class MqttConnectFlag {
	 ...
	 * identifier). If there is no Session associated with the Client identifier 
	 * the Server MUST create a new Session. 
	 * The Client and Server MUST store the Session after the Client and Server 
	 * are disconnected. After the disconnection of a Session that 
	 * had CleanSession set to 0, the Server MUST store further QoS 1 and QoS 2 
	 * messages that match any subscriptions that the client had at the time of 
	 * disconnection as part of the Session state. It MAY also 
	 * store QoS 0 messages that meet the same criteria.
	 * 
    ...
    boolean reserved = false; // The Server MUST validate that the reserved flag in the CONNECT Control Packet is set to zero and disconnect the Client if it is not zero
